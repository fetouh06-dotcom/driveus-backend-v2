...
function getConfig() {
  ...
  // ✅ safer default in production
  const DEFAULT_PROD_ORIGIN = "https://driveus.fr";

  return {
    NODE_ENV,
    PORT: num("PORT", 3000),

    // ✅ avoid empty origin in production
    CORS_ORIGIN:
      process.env.CORS_ORIGIN ||
      (NODE_ENV === "production" ? DEFAULT_PROD_ORIGIN : "*"),
    ...
    // Security: disable public register in production unless explicitly enabled
    ALLOW_PUBLIC_REGISTER: String(process.env.ALLOW_PUBLIC_REGISTER || "").toLowerCase() === "true"
  };
}
