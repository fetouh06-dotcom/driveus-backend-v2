...
router.post("/", async (req, res) => {
  try {
    const { pickup_text, dropoff_text, pickup_datetime, customer_name, customer_phone, customer_email, notes } = req.body || {};
    ...
    const dueAt = new Date(Date.now() + config.DEPOSIT_EXPIRES_MINUTES * 60 * 1000).toISOString();
    ...
    `).run(
      id, pickupLabel, dropoffLabel, distanceKm, price, createdAt, pickup_datetime || null,
      customer_name || null, customer_phone || null, customer_email || null, notes || null,
      config.DEPOSIT_EUR, dueAt
    );

    return res.json(db.prepare("SELECT * FROM bookings WHERE id=?").get(id));
  } catch (e) {
