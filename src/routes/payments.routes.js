...
router.post("/deposit-session", async (req, res) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        ...
        price_data: {
          ...
          product_data: { name: `Acompte DriveUs (${(depositCents/100).toFixed(0)}€)` }
        }
      }],
      metadata: { booking_id },
    ...
    db.prepare("UPDATE bookings SET stripe_session_id=?, payment_status='deposit_pending' WHERE id=?")
      .run(session.id, booking_id);

    return res.json({ url: session.url, session_id: session.id });
  } catch (e) {
    return res.status(e.statusCode || 500).json({ error: e.message || "Erreur paiement (Stripe)" });
  }
});
...
function stripeWebhookHandler(req, res) {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    if (bookingId) {
      db.prepare(`
        UPDATE bookings
        SET deposit_paid=1, payment_status='deposit_paid', status='confirmed',
